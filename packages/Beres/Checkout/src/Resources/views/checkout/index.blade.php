<x-shop::layouts>
    ...
    @push('scripts')
        <script>
            ...
            courier?.addEventListener('change',()=>{if(!courier.value||!cityId.value)return;options.classList.remove('hidden');options.innerHTML='<p class="text-sm text-[#746b66]">Loading shipping services…</p>';fetch('{{ route('shop.checkout.calculate_shipping') }}',{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content,'Content-Type':'application/json'},body:JSON.stringify({courier:courier.value,city_id:cityId.value,weight})}).then(r=>r.json()).then(data=>{options.innerHTML='';(data.data||[]).forEach(group=>(group.costs||[]).forEach(item=>{const label=document.createElement('label');label.className='flex cursor-pointer items-center gap-3 rounded-xl border border-[#e6dfda] p-4 hover:border-[#8d4a3d]';const value=item.cost?.[0]?.value||0;label.innerHTML='<input type="radio" name="shipping_service" value="'+courier.value+'|'+item.service+'" data-cost="'+value+'" class="accent-[#8d4a3d]" required><span class="flex-1"><strong class="block">'+item.service+'</strong><small class="text-[#746b66]">'+(item.description||'')+'</small></span><strong>Rp '+value.toLocaleString('id-ID')+'</strong>';label.querySelector('input').onchange=()=>{method.value=label.querySelector('input').value;cost.value=value;document.getElementById('shipping-cost-display').textContent='Rp '+value.toLocaleString('id-ID');document.getElementById('grand-total').textContent='Rp '+(subtotal+value).toLocaleString('id-ID')};options.appendChild(label)}));if(!options.children.length)options.innerHTML='<p class="text-sm text-red-700">No shipping service is available for this destination.</p>'}).catch(()=>options.innerHTML='<p class="text-sm text-red-700">Shipping calculation failed. Please try again.</p>')});
            form?.addEventListener('submit',async e=>{e.preventDefault();errorBox.classList.add('hidden');const button=document.getElementById('complete-order');button.disabled=true;button.textContent='Processing…';try{const payload=new URLSearchParams(new FormData(form)),session=await fetch('{{ route('shop.checkout.session.store') }}',{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content,'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:payload}).then(async r=>{const d=await r.json();if(!r.ok||!d.success)throw Error(d.message||'Unable to create checkout session.');return d}),order=await fetch('{{ route('shop.checkout.place_order') }}',{method:'POST',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]').content,'Content-Type':'application/json'},body:JSON.stringify({session_id:session.data.id})}).then(async r=>{const d=await r.json();if(!r.ok||!d.success)throw Error(d.message||'Unable to place order.');return d});if(order.payment_url)window.location.href=order.payment_url;else window.location.href='{{ route('shop.checkout.success') }}?order_id='+order.order_id}catch(err){errorBox.textContent=err.message;errorBox.classList.remove('hidden');button.disabled=false;button.textContent='Complete order'}});
        </script>
    @endpush
</x-shop::layouts>
